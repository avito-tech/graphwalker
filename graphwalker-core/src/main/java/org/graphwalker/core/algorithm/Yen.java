...
public class Yen implements Algorithm {
  ...
  public List<Path<Element>> ksp(RuntimeVertex sourceLabel, RuntimeVertex targetLabel, int K) {
    ...
    List<Path<Element>> ksp = new ArrayList<Path<Element>>();
    PriorityQueue<Path<Element>> candidates = new PriorityQueue<>(new Comparator<Path<Element>>() {
      @Override
    ...
    try {
      ...
      for (int k = 1; k < K; k++) {
        ...
        Path<Element> previousPath = ksp.get(k - 1);
